...
export default function OverviewDashboard({
  ...
  const loadStats = useCallback(async () => {
    ...
    // Range lower bound: start of THIS MONTH for the monthly page, or
    // the epoch (effectively "no filter") for the all-time page. Both
    // still implicitly bounded above by "now", same as before.
    const rangeStart = range === 'month'
      ? new Date(today.getFullYear(), today.getMonth(), 1)
      : new Date(0)

    const [rangeCountRes, todayRes, pending, rangeRevenueRes, workers] = await Promise.all([
      supabase
        .from('bookings')
    ...
    const rangeRev = ((rangeRevenueRes.data ?? []) as { final_amount: number }[]).reduce(
